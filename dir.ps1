...
$username = "user"
$password = "pass"
$connectionString = "ip:port/SID" # TNS entry or host:port/service_name
...
"SET ECHO ON`nSET SERVEROUTPUT ON SIZE 1000000`n@""$($file.FullName)""`nLIST`nprompt *******************`nEXIT;" | Out-File -FilePath $tempFile -Encoding ASCII
# Run the SQL file and capture output
    $output = sqlplus -S "$username/$password@$connectionString" "@$tempFile"
